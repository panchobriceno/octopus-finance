...
- ✅ **F3** (aplicado 2026-07-01): catch-up de categorización de TRANSACCIONES históricas Sin categoría/Otros (eran 70, no ~247). `scripts/bank-bot/categorize-historicos.ts`: fase determinista (TASA INT→Intereses bancarios preservando ámbito; reglas curadas→categoría+subcategoría+ámbito) + fase IA (`claude -p`, solo categoría de la lista blanca, NUNCA cambia ámbito, fail-closed). Diseño+revisor por Codex (3 bugs corregidos: SII business→family, apply pisando ámbito editado, proyecciones planned). **Garantía apply==revisado**: el dry-run guarda `_propuestas-historicos.json` y `--apply` aplica exactamente eso (no re-llama la IA, no-determinista) + backup previo + `restore-historicos.ts`. Resultado: **48 aplicadas** (Sin categoría 2→0, Otros 68→18), 3 omitidas por la red de seguridad (cambiaron desde el dry-run), 4 excluidas por Pancho (EL TREBOL=mall ambiguo; 3 TRASPASO DEUDA INTERNACIONAL necesitan `movementType credit_card_payment`+categoría "Tarjeta de credito" para conciliar, que F3 category-only no setea → se editan en `/transactions`). Backup: `_backup-historicos-2026-07-01T15-22-52-701Z.json`. Pendiente manual: ~18 en Otros (no-fit + excluidas) desde Transacciones filtro categoría=Otros.
- **F4-hardening** (estabilizar sourceKey; fuzzy solo como "posible duplicado", no auto-descarte).
